...
- **完全自律型ゲームプレイ**: 最大10体のLLMエージェントが最初から最後までプレイ
...
| `--rounds N` | `2` | 議論ラウンド数 |
| `--games N` | `1` | 連続実行するゲーム数 (CLIのみ)。`>= 2`かつ単語ペアがランダム選択の場合、各ゲームで異なるペアを使用し、全ペアを使い切るまで繰り返さない |
...
| `--html` | オフ | HTMLリプレイ出力を保存。`--games 1`の場合はファイル1つ、`--games >= 2`の場合はゲームごとに1ファイル |
...
| `--json` | オフ | リプレイJSONを保存。`--games 1`の場合はゲーム記録1つ、`--games >= 2`の場合は全ゲームを1つのJSONに集約 |
...
- `--games >= 2 --reflection`を使用すると、各プレイヤーの`lessons_learned`が次のゲームに引き継がれ、プレイのたびに洗練されます。
...
`--continue`を使うと、以前に保存したJSONファイルを読み込んで追加ゲームを実行することで、完了した実験を延長できます。ゲーム設定 (プレイヤー数、狼数、ラウンド数、モデル、言語など)、lessons learned、使用済み単語ペアはすべてJSONから引き継がれます。
...
`--game-id`と`--seed`フィールドは実験追跡のためにJSON出力に含まれます。`--games >= 2`の場合、シードはバッチ開始時に一度だけ適用されるため、シーケンス全体が再現可能です。
...
`src/baml_client/`は自動生成されます — 直接編集しないでください。`.baml`ファイルを変更した後は、以下のコマンドで再生成してください：
